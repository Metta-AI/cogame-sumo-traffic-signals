...
# use-after-free fixed in signals_replay.nim (emscripten_exit_with_live_runtime)
...
# signals_error_len(). Aborting keeps linear memory intact, and the page reads
# signals_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "signals_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_signals_load_replay,_signals_frame,_signals_input,_signals_packet_ptr,_signals_packet_len,_signals_mismatch_tick,_signals_error_ptr,_signals_error_len,_signals_stage_ptr,_signals_stage_len
  """).replace("\n", " ")
)
